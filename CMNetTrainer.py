...
class CMNetTrainer:
    ...
    def get_train_batch(self):
        with tf.device('/cpu:0'):
            ...
            img2, im_coords2 = self.pre_processor.process_train(img2, rel_coords2, 1, True)
    ...
    def train(self):
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                roi_pred_loss += math_ops.add_n(slim.losses.get_regularization_losses())

                # Handle dependencies with update_ops (batch-norm)
                update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
                if update_ops:
                    updates = tf.group(*update_ops)
                    total_loss = control_flow_ops.with_dependencies([updates], roi_pred_loss)
